...
/// Controleur du scroll des tableaux global, permet de garder la position persistante
double globalPositionScroll = 0.0;
int globalPageInfo = 0;
...
class DataTable2 extends DataTable {
  ...
  Widget _buildDataCell(
    ...
    label = Container(
      padding: padding,
      constraints:
          BoxConstraints(minHeight: specificRowHeight ?? effectiveDataRowMinHeight, maxHeight: specificRowHeight ?? effectiveDataRowMaxHeight),
      alignment: numeric ? Alignment.centerRight : AlignmentDirectional.centerStart,
  ...
  Widget build(BuildContext context) {
    ...
    List<TableRow>? fixedColumnsRows = rows.isEmpty
        ...
        : actualFixedColumns > 0
            ? (actualFixedRows < 1
                ? [
                    _buildHeadingRow(context, theme, fixedColumnsColor != null ? WidgetStatePropertyAll(fixedColumnsColor) : effectiveHeadingRowColor,
                        actualFixedColumns),
                    ..._buildTableRows(
                        anyRowSelectable,
    ...
    List<TableRow>? fixedCornerRows = actualFixedColumns > 0 && actualFixedRows > 0
        ? (actualFixedRows == 1
            ...
            : [
                _buildHeadingRow(context, theme, fixedCornerColor != null ? WidgetStatePropertyAll(fixedCornerColor) : effectiveHeadingRowColor,
                    actualFixedColumns),
                ..._buildTableRows(anyRowSelectable, fixedCornerColor != null ? WidgetStatePropertyAll(fixedCornerColor) : effectiveDataRowColor,
                    context, theme, actualFixedColumns, defaultRowColor, null, 0, actualFixedRows - 1, true)
              ])
        : null;

    double checkBoxWidth = _addCheckBoxes(displayCheckboxColumn, effectiveHorizontalMargin, tableColumnWidths, headingRow, effectiveHeadingRowHeight,
        context, someChecked, allChecked, coreRows, fixedRows, fixedCornerRows, fixedColumnsRows, rows, actualFixedRows, effectiveDataRowColor);
              ...
              fixedRowsAndCoreCol = Scrollbar(
                  ...
                  child: Column(mainAxisSize: MainAxisSize.min, children: [
                    ...
                    Flexible(
                        ...
                        child: Scrollbar(
                            thumbVisibility:
                                isVerticalScrollBarVisible ?? (isiOS ? scrollBarTheme.thumbVisibility?.resolve({WidgetState.hovered}) : null),
                            thickness: (isiOS ? scrollBarTheme.thickness?.resolve({WidgetState.hovered}) : null),
    ...
    if (dataTableShowLogs && kDebugMode) {
      ...
      debugPrint('Offset Position verticale : ${globalPositionScroll.toString()}');
      debugPrint('Page Row : ${globalPageInfo.toString()}');
    }
      ...
      (int index) {
        ...
        if (headingRow != null && index == 0) {
          ...
          Decoration? rowDecoration =
              rows[rowStartIndex + actualIndex] is DataRow2 ? (rows[rowStartIndex + actualIndex] as DataRow2).decoration : null;
...
class SyncedScrollControllersState extends State<SyncedScrollControllers> {
  ...
  void _initControllers() {
    ...
    if (widget.scrollController != null) {
      ...
      _sc11 = ScrollController(keepScrollOffset: true);
    }
  ...
  void _saveScrollPosition() {
    if (_sc11 != null && _sc11!.hasClients) {
      globalPositionScroll = _sc11!.offset;
    }
  }

  void _restoreScrollPosition() {
    if (_sc11 != null && _sc11!.hasClients) {
      _sc11!.jumpTo(globalPositionScroll);
    }
  }
...
/// Controle les variables globale de datatable
class CtrlVariableGlobalDatatable {
  static void reset() {
    globalPageInfo = 0;
    globalPositionScroll = 0.0;
  }
}
